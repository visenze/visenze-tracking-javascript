
const SESSION_TIMEOUT = 1800000;
const KEY_UID = "va-uid";
const KEY_SID = "va-key-sid";
const KEY_SID_TIMESTAMP = "va-key-sid-timestamp";

class SessionManager {
    constructor(uid) {
        this.setUID(uid);
    }

    setLocalStorage(key, val) {
        if (typeof (Storage) !== 'undefined') {
            localStorage.setItem(key, val);
        }
    }

    getLocalStorage(key) {
        if (typeof (Storage) !== 'undefined') {
            return localStorage.getItem(key);
        }
        return null;
    }

    removeLocalStorage(key) {
        if (typeof (Storage) !== 'undefined') {
            localStorage.removeItem(key);
        }
    }

    isSameDay(t1, t2) {

        const d1 = new Date(t1);
        const d2 = new Date(t2);
        return d1.getFullYear() === d2.getFullYear()
            && d1.getMonth() === d2.getMonth()
            && d1.getDate() === d2.getDate();
    }

    setUID(uid) {
        if (uid) {
            this.setLocalStorage(KEY_UID, uid);
        }
    }


    getSessionTimestamp() {
        let timestamp = this.getLocalStorage(KEY_SID_TIMESTAMP);
        if (!timestamp) {
            timestamp = new Date().getTime();
            this.setLocalStorage(KEY_SID_TIMESTAMP, timestamp);
        }
        return parseInt(timestamp)
    }


    // get uid from local storage, if uuid is not created yet 
    // create a new uuid and store in localstorage. 
    getUID() {
        let uid = this.getLocalStorage(KEY_UID);
        if (!uid) {
            uid = this.generateUUID();
            this.setLocalStorage(KEY_UID, uid);
        }
        return uid;
    }

    //  get sid from local storage without resetting session timer 
    // if sid is not created yet, then create a new sid and store in local storage
    getSID() {
        const sid = this.getLocalStorage(KEY_SID);
        if (!sid) {
            return this.getSessionId();
        }

        return sid;
    }

    generateUUID() {
        let d = new Date().getTime();
        const uuid = 'xxxxxxxx.xxxx.4xxx.yxxx.xxxxxxxxxxxx'.replace(/[xy]/g, (c) => {
            const r = (d + Math.random() * 16) % 16 | 0;
            d = Math.floor(d / 16);
            return (c === 'x' ? r : (r & 0x3 | 0x8)).toString(16);
        });
        return uuid;
    }

    // if sid is not created yet, then create a new sid and store in local storage
    getSessionId() {
        let now = new Date().getTime();
        let sid = this.getLocalStorage(KEY_SID);
        let sessionTimestamp = this.getSessionTimestamp();


        if (!sid || (now - sessionTimestamp) > SESSION_TIMEOUT || !this.isSameDay(now, sessionTimestamp)) {
            sid = this.generateUUID();
            this.setLocalStorage(KEY_SID, sid);
        }

        this.setLocalStorage(KEY_SID_TIMESTAMP, now);
        return sid;
    }

    resetSession() {
        this.removeLocalStorage(KEY_SID);
        this.removeLocalStorage(KEY_SID_TIMESTAMP);
        return this.getSessionId();
    }

    getSessionTimeRemaining() {
        let now = new Date().getTime();
        let sessionTimestamp = this.getLocalStorage(KEY_SID_TIMESTAMP);

        if (!sessionTimestamp) {
            return 0;
        }

        sessionTimestamp = parseInt(sessionTimestamp);
        if (!this.isSameDay(now, sessionTimestamp) || (now - sessionTimestamp) > SESSION_TIMEOUT) {
            return 0;
        }

        return SESSION_TIMEOUT - (now - sessionTimestamp);
    }
}

module.exports = SessionManager;